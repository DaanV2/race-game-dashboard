...
{ 
    ...
    CarMotionData   m_carMotionData[24];     // Data for all cars on track 
};
...
// 5 = Medium Long, 6 = Long, 7 = Full 
    ...
    float    m_startReactionTime;                // Driver start reaction time in seconds 
     // 0.0f if assisted starts 
    ...
    uint8    m_dynamicRacingLineColourBlind;     // 0 = Off, 1 = Protanopia, 2 = Deuteranopia 
     // 3 = Tritanopia 
...
{ 
    ...
    LapData         m_lapData[24];         // Lap data for all cars on track 
...
{ 
    ...
    uint8           m_numActiveCars; // Number of active cars in the data – should match number of 
                                         // cars on HUD 
    ParticipantData m_participants[24]; 
...
{ 
    ...
    CarTelemetryData    m_carTelemetryData[24]; 
...
{ 
    ...
    CarStatusData m_carStatusData[24]; 
}; 
...
{ 
    ...
    FinalClassificationData    m_classificationData[24]; 
}; 
...
{ 
    ...
    LobbyInfoData       m_lobbyPlayers[24]; 
}; 
...
{ 
    ...
    CarDamageData   m_carDamageData[24]; 
}; 
...
{ 
    ...
    uint8           m_positionForVehicleIdx[50][24]; 
};
...
{ 
    ...
    uint16    m_activeAeroActivationDistance;     // 0 = Active aero not available, non-zero – Active 
 // aero will be available in [X] metres 
    uint8     m_overtakeAvailable;                // 0 = not available, 1 = available 
    uint8     m_overtakeActive;                   // 0 = not active, 1 = active 
    uint16    m_overtakeActivationDistance;       // 0 = Overtake Mode not available, non-zero – 

    uint8     m_2026Regulations;                // 0 = vehicle conforms to pre-2026, 1 = 2026  
